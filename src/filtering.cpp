 ...
 *
 * Assumes the model is homogeneous.
 *
...
int main(int argc, char* argv[]) {
  ...
  for (unsigned int s=0; s<D.size(); ++s) {
	...
	// run algorithm
	pair<NUM,NUM> prob = filtering(q,Al,Au,Bl,Bu,Pl,Pu,O,N,M,K,epsilon,debug,opt);
	// output result
	cout << scientific << "PROB(Q" << K << "=" << q << "|DATA) in [" << prob.first << ", " << prob.second << "]" << endl;
